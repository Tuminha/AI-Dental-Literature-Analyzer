...
import shlex
from ai_scientist.analyze_dental_literature import main as analyze_dental_literature
import anthropic
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help='Folder containing input PDFs for dental literature review'
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_folder', type=str, required=False, help='Folder containing input PDFs for dental literature review')
    args = parser.parse_args()

    if args.experiment == 'dental_literature_review':
        if not args.input_folder:
            parser.error("--input_folder is required for dental_literature_review experiment")
        
        input_folder = args.input_folder.strip()
        out_dir = f"results/dental_literature_review/run_{int(time.time())}"
        
        pdf_files = [f for f in os.listdir(input_folder) if f.endswith('.pdf')]
        
        print(f"Found {len(pdf_files)} PDF files for analysis.")
        
        with alive_bar(len(pdf_files), title="Overall Progress", theme='smooth') as bar:
            for pdf_file in pdf_files:
                pdf_path = os.path.join(input_folder, pdf_file)
                print(f"\nAnalyzing: {pdf_file}")
                analyze_dental_literature(pdf_path, out_dir, args.model, get_client(args.model))
                bar()

        print(f"\nAll analyses complete. Results saved in:")
        print(f"JSON: {os.path.abspath(out_dir)}")
        print(f"Markdown: /Users/franciscoteixeirabarbosa/Dropbox/Random scripts/The AI Scientist/results_in_md")
